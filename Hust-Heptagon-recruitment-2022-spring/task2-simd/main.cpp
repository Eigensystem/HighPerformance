...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  for (i = 0; i < N - VECTOR_WIDTH; i += VECTOR_WIDTH) 
  {
    ...
    _cs149_vload_int(index, exponents + i, maskAll);
    y = _cs149_vset_float(1.00);
    _cs149_veq_int(maskexp, index, zero, maskAll);
    maskexp = _cs149_mask_not(maskexp);
    do
    {
      _cs149_vmult_float(y, x, y, maskexp);
      _cs149_vsub_int(index, index, one, maskexp);
      _cs149_veq_int(maskexp, index, zero, maskAll);
      maskexp = _cs149_mask_not(maskexp);
    } while (_cs149_cntbits(maskexp));
    _cs149_vgt_float(maskexp, y, bound, maskAll);
    _cs149_vmove_float(y, bound, maskexp);
    _cs149_vstore_float(output + i, y, maskAll);
  }
  ...
  _cs149_veq_int(maskexp, index, zero, maskAll);
  maskexp = _cs149_mask_not(maskexp);
  maskexp = _cs149_mask_and(maskexp, maskAll);
  do
  {
    _cs149_vmult_float(y, x, y, maskexp);
    _cs149_vsub_int(index, index, one, maskexp);
...
float arraySumVector(float* values, int N) {
  ...
  //
  __cs149_vec_float sum_arr = _cs149_vset_float(0);
  __cs149_vec_float x = _cs149_vset_float(0);
  __cs149_mask maskAll = _cs149_init_ones();

  for (int i=0; i < N; i+=VECTOR_WIDTH) {
    _cs149_vload_float(x, values + i, maskAll);
    _cs149_vadd_float(sum_arr, sum_arr, x, maskAll);
    // if(i == N - VECTOR_WIDTH) { 
  ...
  for(int i = 1; i < VECTOR_WIDTH; i *= 2) {
    _cs149_hadd_float(sum_arr, sum_arr);
    _cs149_interleave_float(sum_arr, sum_arr);
  }
  return sum_arr.value[0];
}
